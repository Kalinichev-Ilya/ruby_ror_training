...
class Program
  ...
  def create_route_interface
    ...
    from_start = gets.chomp.to_i
    puts 'Select the end station'
    to_end = gets.chomp.to_i
    create_route(@stations[from_start], @stations[to_end])
    show_list(@routes)
  end
  ...
  def create_train_interface
    ...
    puts "Train number #{train.number} was successfully created." unless train.nil?
  end
  ...
  def create_route(from_start, to_end)
    @routes << Route.new(from_start, to_end)
  rescue ValidationError => e
    puts "#{e.message}: #{e.obj}"
  end
